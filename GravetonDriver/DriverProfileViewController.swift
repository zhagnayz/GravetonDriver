...
class DriverProfileViewController: UIViewController{
    ...
    override func viewDidLoad() {
        ...
        let ref = Database.database().reference().child("Drivers")
        ...
        if let uid = Auth.auth().currentUser?.uid {
            ...
            ref.child(uid).observeSingleEvent(of: .value) { snapshot,error  in
                if let dictionary = snapshot.value as? [String : Any] {
                    ...
                    let person = [IconMenu(icon: "person.circle", title: name ?? "", subTitle: nil),IconMenu(icon: "envelope.circle.fill", title: email ?? "", subTitle: nil),IconMenu(icon: "phone.circle.fill", title: phone ?? "", subTitle: nil),IconMenu(icon: nil, title: "Delete Account", subTitle: nil)]
                    self.person = person
                    self.reloadData()
                }
            }
...
extension DriverProfileViewController: UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if indexPath.item == 3 {
            createAlernController("Deleting Account", "will clear all related information to you.", "Yes", "No")
        }
    }
}
