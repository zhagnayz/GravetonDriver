...
class DropViewController: UIViewController{
    ...
    private let value:String = "Task Details"
    ...
    override func viewDidLoad() {
        ...
        if user?.driverInfo?.pickUpOrDrop == "PickUp" {
            
            floatingButton.setTitle("Hold to done pick up", for: .normal)
            if let orderId = user?.orderId {
                status_ref = Database.database().reference().child("orders").child(orderId)
                status_ref?.updateChildValues(["current":"Driver heading to store"])
            }
        }else{
            if let orderId = user?.orderId {
                status_ref = Database.database().reference().child("orders").child(orderId)
        ...
        navigationItem.leftBarButtonItem = UIBarButtonItem(image: UIImage(systemName: "chevron.right"), style: .done, target: self, action: nil)
        ...
        NSLayoutConstraint.activate([
            floatingButton.leadingAnchor.constraint(equalTo: view.leadingAnchor),
    ...
    func createDataSource(){
        ...
        dataSource = UICollectionViewDiffableDataSource<Sections,AnyHashable>(collectionView:collectionView){collectionView,indexPath, item in
            ...
            case Sections.sectionOne.rawValue:
                ...
                if self.user?.driverInfo?.pickUpOrDrop == "PickUp" {
            ...
            case Sections.sectionThree.rawValue:
                ...
                cell.configureCell(self.user)
                cell.taskDetailLabel.text = self.value
            ...
            if user?.driverInfo?.pickUpOrDrop == "PickUp" {
                ...
                if sender.state == .ended {
                    ...
                    if numberPassed == 1 {
                        ...
                        sender.view?.backgroundColor = colordrop[0]
                ...
                if sender.state == .ended {
                    ...
                    if numberPassed == 1 {
                        ...
                        button.setTitle("Hold to done", for: .normal)
